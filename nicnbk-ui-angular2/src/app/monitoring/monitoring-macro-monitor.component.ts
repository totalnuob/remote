...
export class MonitoringMacroMonitorComponent extends GoogleChartComponent  {
    ...
    drawGraph(){
        this.drawValuesTable('table', this.data);
        this.drawLineChart('line_chart', this.data, 'MSCI WORLD');

        this.drawValuesTable('table_us', this.dataUs);
        this.drawLineChart('line_chart_us', this.dataUs, 'S&P 500');
    }


    drawValuesTable(id, input){
    ...
    drawLineChart(id, input, str){
        var data = new google.visualization.DataTable();
        data.addColumn("string", "Date");
        data.addColumn("number", "Macro Score");
        data.addColumn("number", str);

        data.addRows(this.getLineChartData(input));
    ...
    private getLineChartData(input){
        var rows = [];
        for(var i = this.dates.length - 1; i >= 0; i--){
            var row = [this.dates[i], input[8][1+i], input[9][1+i]];
            rows.push(row);
        }
        //console.log(rows);
        return rows;
