  INSERT INTO strategy(id, code, name_en, name_ru, name_kz, group_type) VALUES(1, 'MEGA_CAP', 'Mega/Large Cap Buyout', '', '', 1);
  INSERT INTO strategy(id, code, name_en, name_ru, name_kz, group_type) VALUES(2, 'MID_CAP', 'Mid Cap Buyout', '', '' , 1);
  INSERT INTO strategy(id, code, name_en, name_ru, name_kz, group_type) VALUES(3, 'SM_CAP', 'Small Cap Buyout', '', '' , 1);
  INSERT INTO strategy(id, code, name_en, name_ru, name_kz, group_type) VALUES(4, 'PE_CREDIT', 'Credit', '', '' , 1);
  INSERT INTO strategy(id, code, name_en, name_ru, name_kz, group_type) VALUES(5, 'PE_MULTI', 'Multi', '', '' , 1);
  INSERT INTO strategy(id, code, name_en, name_ru, name_kz, group_type) VALUES(6, 'PE_OTHER', 'Other', '', '' , 1);

  INSERT INTO strategy(id, code, name_en, name_ru, name_kz, group_type) VALUES(21, 'EQUITY', 'Equity', '', '', 2);
  INSERT INTO strategy(id, code, name_en, name_ru, name_kz, group_type) VALUES(22, 'EVENT', 'Event Driven', '', '', 2);
  INSERT INTO strategy(id, code, name_en, name_ru, name_kz, group_type) VALUES(23, 'REL_VAL', 'Relative Value', '', '', 2);
  INSERT INTO strategy(id, code, name_en, name_ru, name_kz, group_type) VALUES(24, 'MACRO', 'Macro/Directional', '', '' , 2);
  INSERT INTO strategy(id, code, name_en, name_ru, name_kz, group_type) VALUES(25, 'HF_CREDIT', 'Credit', '', '', 2);
  INSERT INTO strategy(id, code, name_en, name_ru, name_kz, group_type) VALUES(26, 'COMMOD', 'Commodities', '', '', 2);
  INSERT INTO strategy(id, code, name_en, name_ru, name_kz, group_type) VALUES(27, 'HF_MULTI', 'Multistrategy', '', '' , 2);
  INSERT INTO strategy(id, code, name_en, name_ru, name_kz, group_type) VALUES(28, 'LONGBIAS', 'Long Biased', '', '' , 2);

  INSERT INTO strategy(id, code, name_en, name_ru, name_kz, group_type) VALUES(41, 'CORE', 'Core', '', '', 3);
  INSERT INTO strategy(id, code, name_en, name_ru, name_kz, group_type) VALUES(42, 'CORE+', 'Core+', '', '', 3 );
  INSERT INTO strategy(id, code, name_en, name_ru, name_kz, group_type) VALUES(43, 'VAL_ADD', 'Value Added', '', '', 3);
  INSERT INTO strategy(id, code, name_en, name_ru, name_kz, group_type) VALUES(44, 'OPPORTUN', 'Opportunistic', '', '', 3 );
  INSERT INTO strategy(id, code, name_en, name_ru, name_kz, group_type) VALUES(45, 'RE_MULTI', 'Multi', '', '' , 3);
  INSERT INTO strategy(id, code, name_en, name_ru, name_kz, group_type) VALUES(46, 'RE_OTHER', 'Other', '', '' , 3);