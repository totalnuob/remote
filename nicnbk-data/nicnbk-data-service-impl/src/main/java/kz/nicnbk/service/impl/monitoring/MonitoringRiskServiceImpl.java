...
import kz.nicnbk.repo.api.risk.AllocationByTopPortfolioRepository;
import kz.nicnbk.repo.api.risk.MonitoringRiskHFMonthlyReportRepository;
import kz.nicnbk.repo.api.risk.MonitoringRiskHFPortfolioReturnRepository;
...
public class MonitoringRiskServiceImpl implements MonitoringRiskService {
    ...
    public MonitoringRiskHedgeFundReportDto getMonthlyHedgeFundRiskReport(MonitoringRiskReportSearchParamsDto searchParamsDto){
        ...
        Iterator<MonitoringRiskHFPortfolioReturn> returnsIterator = this.riskHFReturnRepository.findAll().iterator();
        if(returnsIterator != null){
            while(returnsIterator.hasNext()){
                MonitoringRiskHFPortfolioReturn value = returnsIterator.next();
...
//                    }
    ...
    private ListResponseDto getHedgeFundsFactorBetas(Date date, NicPortfolioResultDto nicPortfolioResultDto){
        ...
        // Global FI Unhedged
//        setFactorBeta(responseDto, records, dateFromSinceInception, dateFrom12M, dateTo, benchmarksMap.get(BenchmarkLookup.T_BILLS.getCode()), benchmarksMap.get(BenchmarkLookup.T_BILLS.getCode() + "_SI"),
//                "Barclays Global Agg Index Unhedged", BenchmarkLookup.GLOBAL_FI_U.getCode(), benchmarksMap.get(BenchmarkLookup.GLOBAL_FI_U.getCode()), benchmarksMap.get(BenchmarkLookup.GLOBAL_FI_U.getCode() + "_SI"), nicPortfolioResultDto);
        // US High Yields 12M
        setFactorBeta(responseDto, records, dateFromSinceInception, dateFrom12M, dateTo, benchmarksMap.get(BenchmarkLookup.T_BILLS.getCode()), benchmarksMap.get(BenchmarkLookup.T_BILLS.getCode() + "_SI"),
