...
public class EmployeeServiceREST extends CommonServiceREST{
    ...
    @RequestMapping(value = "/findCorpMembers", method = RequestMethod.GET)
    public ResponseEntity findCorpMembers(){
        List<EmployeeDto> employees = this.employeeService.findCorpMembers();
        if(employees == null){
            // error occurred
            return new ResponseEntity<>(null, null, HttpStatus.INTERNAL_SERVER_ERROR);
        }else{
